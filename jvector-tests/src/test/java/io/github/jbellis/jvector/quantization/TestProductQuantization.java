...
public class TestProductQuantization extends RandomizedTest {
    ...
    public void testPQCodebookSums() {
        ...
        var sf = pqm.diversityFunctionFor(10, VectorSimilarityFunction.EUCLIDEAN);

        assertEquals(1, sf.similarityTo(10), 1e-6);
        ...
        var sf2 = pqi.diversityFunctionFor(10, VectorSimilarityFunction.EUCLIDEAN);
        assertEquals(1, sf2.similarityTo(10), 1e-6);

        for (int i = 0; i < vectors.size(); i++) {
            assertEquals(sf.similarityTo(i), sf2.similarityTo(i), 1e-6);
        }
    }
}
